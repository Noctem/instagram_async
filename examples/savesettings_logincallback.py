...
def onlogin_callback(api, new_settings_file):
    ...
    with open(new_settings_file, 'w') as outfile:
        ...
        print(f'SAVED: {new_settings_file}')
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f'ClientLoginError {e}')
        exit(9)
    except ClientError as e:
        print(f'ClientError {e.msg} (Code: {e.code}, Response: {e.error_response})')
        exit(9)
    ...
    print(f"Cookie Expiry: {datetime.datetime.fromtimestamp(cookie_expiry).strftime('%Y-%m-%dT%H:%M:%SZ')}")
